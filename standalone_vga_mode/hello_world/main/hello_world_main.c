...
void IRAM_ATTR video_isr(volatile void*);
...
static esp_err_t start_dma(int line_width,int samples_per_cc, int ch)
{
  ...
  periph_module_enable(PERIPH_I2S0_MODULE);
    ...
    if (!_pal_) {
      printf("enabled the samples for ntsc\n");
    ...
    if (dac_output_enable(DAC_CHANNEL_1)!=ESP_OK) printf("Error configuring DAC\n");           // DAC, video on GPIO25
    if (dac_i2s_enable()!=ESP_OK)printf("Error enabling i2s/dac\n");                           // start DAC!
...
{
  /* reg = LEDC.channel_group[0].channel[0];
...
void clear_screen();
    void init_screen()
    {
        Screen = (uint32_t*)MALLOC32(Screen_WIDTH*Screen_HEIGHT,"Screen_atari");    // 32 bit access plz
        
        _lines = (uint8_t**)MALLOC32(Screen_HEIGHT*sizeof(uint8_t*),"_lines");
        const uint8_t* s = (uint8_t*)Screen;
	...
	atari_4_phase_ntsc_ram = (uint32_t *) malloc(sizeof(uint32_t) *256);
    ...
    const uint32_t* pal_palette()
    {
     ...
     const uint32_t* rgb_palette()   { return atari_palette_rgb; };

void start_pump() {
  video_init(4,EMU_ATARI,ntsc_palette(),NTSC);                                     
}
...
void app_main()
{
	...
	colors++;
	_lines[yy][xx] = atari_4_phase_ntsc_ram[colors%250];
      }
    while (1){
      //if (blit_flag %5000 == 0)
      //printf("5,000 blits\n");
    
      // if (i2s_intr_flag % 8000==0)
      //printf("8,000 i2s ir\n");
      //vTaskDelay(100);
    }
    //esp_restart();
